...
export class ImageHolder {
    ...
    //load images
    loadImages(tileList: TileList) {
        console.log("Loading images...")

        this.tileList = tileList

        for (let imageUrl of this.imagesToLoad) {
            this.images[imageUrl] = new Image()
            this.images[imageUrl].addEventListener("load", () => {
                this.loadedImages += 1
                if (this.loadedImages >= this.imagesToLoad.length) {
                    this.tileList.updateSearch()
                    this.tileList.updateItemSearch()
